...
SELECT CUST.customer_unique_id AS Customer, CUST.customer_city AS City, CUST.customer_state AS "State", COUNT(order_id) AS TotalOrder
FROM olist_orders_dataset O
JOIN olist_customers_dataset CUST ON O.customer_id = CUST.customer_id
GROUP BY Customer, City, "State"
...
SELECT MONTH(O.order_purchase_timestamp) AS "Month", COUNT(DISTINCT OIT.seller_id) AS "Number of Seller"
FROM olist_order_items_dataset OIT
JOIN olist_orders_dataset O ON OIT.order_id = O.order_id
...
SELECT
    DISTINCT CUS.customer_state AS "State",
    ROUND(AVG(OIT.price),2) AS "Average Sales",
    COUNT(O.order_id) AS "Total Order"
FROM olist_orders_dataset O
JOIN olist_order_items_dataset OIT ON O.order_id = OIT.order_id
JOIN olist_customers_dataset CUS ON CUS.customer_id = O.customer_id
GROUP BY "State"
HAVING "Total Order" > 500
